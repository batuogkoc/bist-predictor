...
class SimpleMLP(nn.Module):
    def __init__(self, seq_len: int, hidden_size: int = 1024, num_layers: int = 8, dropout: float = 0.1):
        super().__init__()
        layers = []
        input_size = seq_len
...
class SimpleRNN(nn.Module):
    def __init__(self, seq_len: int, rnn_hidden: int = 64, num_layers: int = 1, bidirectional: bool = False, dropout: float = 0.0):
        super().__init__()
        self.rnn = nn.GRU(
            input_size=1,
        ...
        self.fc = nn.Linear(rnn_hidden * factor, 2)

    def forward(self, x: torch.Tensor) -> torch.Tensor:
        # Expect x shape: (batch, seq_len) or (batch, seq_len, 1)
        if x.dim() == 2:
            x = x.unsqueeze(-1)
        out, _ = self.rnn(x)
        # take last time-step
